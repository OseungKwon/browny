...
export default function ShopTagFiltered({ formik, filters, isShowReset, onResetFilter }) {
  ...
  return (
    <RootStyle>
      {gender.length > 0 && (
        <WrapperStyle>
          ...
          <Stack direction="row" flexWrap="wrap" sx={{ p: 0.75 }}>
            {gender.map((_gender) => (
              <Chip key={_gender} label={_gender} size="small" onDelete={() => handleRemoveGender(_gender)} sx={{ m: 0.5 }} />
            ))}
          </Stack>
        </WrapperStyle>
      )}
      ...
      {isShow && (
        <MButton color="error" size="small" type="button" onClick={onResetFilter} startIcon={<Icon icon={roundClearAll} />}>
          Clear All
        </MButton>
      )}
    </RootStyle>
