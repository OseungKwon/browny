    ...
    <Draggable draggableId={card.id} index={index}>
      {(provided) => (
        <div {...provided.draggableProps} {...provided.dragHandleProps} ref={provided.innerRef}>
          ...
          >
            <Box onClick={handleOpenDetails} sx={{ cursor: 'pointer' }}>
              {attachments.length > 0 && (
                ...
                >
                  <Box component="img" src={attachments[0]} sx={{ position: 'absolute', top: 0, width: 1, height: 1 }} />
                </Box>
              )}
          ...
          <KanbanTaskDetails card={card} isOpen={openDetails} onClose={handleCloseDetails} onDeleteTask={() => onDeleteTask(card.id)} />
        </div>
      )}
    </Draggable>
  );
