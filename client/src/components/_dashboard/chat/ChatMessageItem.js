...
export default function ChatMessageItem({ message, conversation, onOpenLightbox, ...other }) {
  ...
  const senderDetails =
    message.senderId === '8864c717-587d-472a-929a-8e5f298024da-0' ? { type: 'me' } : { avatar: sender.avatar, name: sender.name };
  ...
  return (
    <RootStyle {...other}>
      ...
      >
        {senderDetails.type !== 'me' && <Avatar alt={senderDetails.name} src={senderDetails.avatar} sx={{ width: 32, height: 32 }} />}
